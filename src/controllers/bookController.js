...
const getBooksQuery = async (req, res) => {
  try {
    ...
    //existsBooks
    const existsBooks = await bookModel.find({ $and: [{ isDeleted: false }, reqBody] }).select({ title: 1, excerpt: 1, userId: 1, category: 1, releasedAt: 1, reviews: 1 }).sort({ title: 1 });

    if (existsBooks.length === 0) return res.status(404).send({ status: false, message: `Book not found.` });
